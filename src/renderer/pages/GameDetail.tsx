...
function StatItem({ label, value, tooltip, color }: { label: string; value: string; tooltip: string; color?: string }) {
  return (
    <Tooltip text={tooltip} position="top">
      <div className="gd-stat-item">
        <span className="gd-stat-label">{label}</span>
        <span className="gd-stat-value" style={color ? { color } : undefined}>
          {value}
        </span>
      </div>
    </Tooltip>
...
function SignatureStats({ json }: { json: string }) {
  ...
  return (
    <div className="card gd-section">
      <h3 className="gd-section-title">Signature Stats</h3>
      <p className="gd-sig-caveat">These are approximations from conversion sequences, not frame-perfect detections.</p>
    ...
    <div className="page-container gd-page">
      ...
      >
        ...
        <div className="gd-actions">
          <button className="btn btn-accent" onClick={() => setShowCoachingModal(true)}>
            Get Coaching
          </button>
          <button className="btn" onClick={() => window.clippi.openInDolphin(game.replayPath)}>
            Watch Replay
          </button>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="gd-stat-grid">
          <StatItem
            ...
            value={pct(game.neutralWinRate)}
            tooltip="First hit in each exchange. Above 50% = winning neutral."
            color={statColor(game.neutralWinRate, 0.5)}
          />
          <StatItem
            label="L-Cancel Rate"
            value={pct(game.lCancelRate)}
            tooltip="L-cancel success rate. Target: 90%+."
            color={statColor(game.lCancelRate, 0.9, 0.7)}
          />
          <StatItem
            label="Conversion Rate"
            value={pct(game.conversionRate)}
            tooltip="Openings that led to follow-up damage."
            color={statColor(game.conversionRate, 0.6, 0.3)}
          />
          <StatItem
            label="Openings/Kill"
            value={fixed1(game.openingsPerKill)}
            tooltip="Neutral wins needed per kill. Lower = better punish game."
            color={statColor(1 / Math.max(game.openingsPerKill, 0.01), 0.2)}
          />
          <StatItem
            label="Avg Dmg/Opening"
            value={fixed1(game.avgDamagePerOpening)}
            tooltip="Average damage dealt per neutral win."
          />
          <StatItem
            label="Kill Conversions"
            value={String(game.killConversions)}
            tooltip="Number of conversions that resulted in a stock."
          />
          <StatItem
            label="Counter Hits"
            value={String(game.counterHits)}
            tooltip="Times you hit opponent during their attack animation."
          />
      ...
      >
        ...
        <div className="gd-stat-grid">
          ...
          <StatItem
            label="Dash Dance Frames"
            value={String(game.dashDanceFrames)}
            tooltip="Frames spent dash dancing."
          />
          <StatItem
            label="Avg Stage Pos X"
            value={fixed1(game.avgStagePositionX)}
            tooltip="Average horizontal position. 0 = center stage."
          />
      ...
      >
        ...
        <div className="gd-stat-grid">
          <StatItem
            label="Recovery Rate"
            value={pct(game.recoverySuccessRate)}
            tooltip="Successful recoveries / attempts."
            color={statColor(game.recoverySuccessRate, 0.7, 0.4)}
          />
          <StatItem label="Recovery Attempts" value={String(game.recoveryAttempts)} tooltip="Times knocked offstage." />
          <StatItem
            label="Avg Death %"
            value={`${fixed1(game.avgDeathPercent)}%`}
            tooltip="Average percent at death. Higher = better DI/survival."
          />
          <StatItem
            label="DI Survival"
            value={fixed1(game.diSurvivalScore)}
            tooltip="Estimated DI quality when surviving hits. Higher = better."
            color={statColor(game.diSurvivalScore, 0.7, 0.3)}
          />
          <StatItem
            label="DI Combo"
            value={fixed1(game.diComboScore)}
            tooltip="Estimated DI quality when escaping combos. Higher = better."
            color={statColor(game.diComboScore, 0.7, 0.3)}
          />
          <StatItem label="Dmg Taken" value={fixed1(game.totalDamageTaken)} tooltip="Total damage received." />
          <StatItem
            label="Power Shields"
            value={String(game.powerShieldCount)}
            tooltip="Frame-perfect shield inputs."
          />
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="gd-stat-grid">
          <StatItem
            label="Edgeguard Rate"
            value={pct(game.edgeguardSuccessRate)}
            tooltip="Successful edgeguards / attempts."
            color={statColor(game.edgeguardSuccessRate, 0.6, 0.3)}
          />
          <StatItem
            label="Edgeguard Attempts"
            value={String(game.edgeguardAttempts)}
            tooltip="Times you went offstage to edgeguard."
          />
          <StatItem
            label="Shield Pressure"
            value={String(game.shieldPressureSequences)}
            tooltip="Multi-hit shield pressure sequences."
          />
          <StatItem
            label="Shield Avg Dmg"
            value={fixed1(game.shieldPressureAvgDamage)}
            tooltip="Average damage per shield pressure sequence."
          />
          <StatItem
            label="Shield Breaks"
            value={String(game.shieldBreaks)}
            tooltip="Times you broke opponent's shield."
          />
          <StatItem
            label="Shield Poke Rate"
            value={pct(game.shieldPokeRate)}
            tooltip="Hits that poked through partial shield."
          />
      ...
      >
        ...
        <div className="gd-stat-grid">
          <StatItem
            label="Ledge Options"
            value={entropyLabel(game.ledgeEntropy)}
            tooltip={`Entropy: ${fixed1(game.ledgeEntropy)}. Higher = more varied ledge getup options.`}
          />
          <StatItem
            label="Tech Options"
            value={entropyLabel(game.knockdownEntropy)}
            tooltip={`Entropy: ${fixed1(game.knockdownEntropy)}. Higher = more varied knockdown tech choices.`}
          />
          <StatItem
            label="Shield Pressure"
            value={entropyLabel(game.shieldPressureEntropy)}
            tooltip={`Entropy: ${fixed1(game.shieldPressureEntropy)}. Higher = more varied shield pressure options.`}
          />
        </div>
      </motion.div>
      ...
      {highlights.length > 0 && (
        ...
        >
          ...
          <HighlightCards highlights={highlights} replayPath={game.replayPath} maxVisible={20} />
        </motion.div>
      )}
        ...
        >
          ...
          <div className="gd-analyses">
            {game.coachingAnalyses.map((a) => (
              <div key={a.id} className="card gd-analysis-card">
                <button
                ...
                >
                  ...
                  <ChevronDown size={14} className={`gd-chevron ${expandedAnalysis === a.id ? "open" : ""}`} />
                </button>
                <AnimatePresence>
